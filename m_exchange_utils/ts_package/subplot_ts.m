function [] = subplot_ts(ax,idx,ytix,lbl,varargin)
...
% spacing (hard coded here, feel free to change):
%        0 = each axes uses whole range (no spacing)
...
varargin{s1ind+2}=varargin{s1ind+2}.*flipscale-10*idx-mdp+5;
if s2ind~=0
    varargin{s2ind+2}=varargin{s2ind+2}.*flipscale-10*idx-mdp+5;
end
if s3ind~=0
    varargin{s3ind+2}=varargin{s3ind+2}.*flipscale-10*idx-mdp+5;
end
if s4ind~=0
    varargin{s4ind+2}=varargin{s4ind+2}.*flipscale-10*idx-mdp+5;
end
